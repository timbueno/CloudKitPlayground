  ...
  private func createPrivateSubscriptionsIfNeeded() {
    ...
    operation.modifySubscriptionsCompletionBlock = { [weak self] _, _, error in
      ...
      if let error = error {
        ...
        error.retryCloudKitOperationIfPossible(self.log, queue: self.workQueue) { self.createPrivateSubscriptionsIfNeeded() }
      } else {
  ...
  private func checkSubscription() {
    ...
    operation.fetchSubscriptionCompletionBlock = { [weak self] ids, error in
      ...
      if let error = error {
        ...
        if !error.retryCloudKitOperationIfPossible(self.log, queue: self.workQueue, with: { self.checkSubscription() }) {
