  ...
  private func createCustomZoneIfNeeded() {
    ...
    operation.modifyRecordZonesCompletionBlock = { [weak self] _, _, error in
      ...
      if let error = error {
        ...
        error.retryCloudKitOperationIfPossible(self.log, queue: self.workQueue) { self.createCustomZoneIfNeeded() }
      } else {
  ...
  private func checkCustomZone() {
    ...
    operation.fetchRecordZonesCompletionBlock = { [weak self] ids, error in
      ...
      if let error = error {
        ...
        if !error.retryCloudKitOperationIfPossible(self.log, queue: self.workQueue, with: { self.checkCustomZone() }) {
